...
import {
  ...
  function isProbablyPolish(input) {
    ...
    try {
      ...
      // If we have Polish characters, we can be pretty confident it's Polish
      if (hasPolishChars) {
        console.log('[SECURITY] Polish characters detected in input');
        return true;
      }
      
      // As a fallback, check for common Polish words
      const hasPolishWords = /\b(jest|nie|tak|co|jak|gdzie|kto|to|czy|ale|dla|przez|się|jestem|będę|mam|chcę)\b/i.test(input);
      
      if (hasPolishWords) {
        console.log('[SECURITY] Common Polish words detected');
        return true;
      }
